...
import { AddSubjectForm } from "../../../../layouts/forms/AddSubjectForm";
...
export const MasterSubjects = () => {
  ...
  const handleLoad = async () => {
    try {
      setIsLoading(true);
      const res = await fetchAPI(`/subjects/all`);
  ...
  const handleSubmit = async () => {
    ...
    try {
      setIsLoading(true);
      setIsSuccess(false);
      const res = await postAPI(`/subjects`, "POST", values);
      setIsSuccess(true);
  ...
  return (
    <>
      <DashboardLayout name={"Manajemen Mata Kuliah"}>
        <Fab
          sx={fabStyle}
          color="primary"
          aria-label="add"
